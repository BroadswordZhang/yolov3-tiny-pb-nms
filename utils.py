...
def detections_boxes(detections):
    ...
    detections = tf.squeeze(detections,name="output_boxes")
    # detect_1 = tf.identity(detect_1, name='detect_1')
    # detect_2 = tf.identity(detect_2, name='detect_2')
    return detections


def detections_boxes_2(detect_1,detect_2):
    """
    Converts center x, center y, width and height values to coordinates of top left and bottom right points.
...
def plot_one_box_label(img, coord, label=None, color=None, line_thickness=None):
    '''
    coord: [x_min, y_min, x_max, y_max] format coordinates.
    img: img to plot on.
    label: str. The label name.
    ...
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label[0], 0, fontScale=float(tl) / 3, thickness=tf)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, color, -1)  # filled
        cv2.putText(img, label[0]+label[1], (c1[0], c1[1] - 2), 0, float(tl) / 3, [0, 0, 0], thickness=tf, lineType=cv2.LINE_AA)